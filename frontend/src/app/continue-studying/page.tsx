...
import { supabase, getRecentStudyActivity, trackDocumentStat } from "../lib/api";
          ...
          return (
            <article key={doc.id} className="group flex flex-col rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:border-indigo-500 dark:border-[#1F2A44] dark:bg-[#111827]">
              ...
              <div className="mt-4 flex gap-2 border-t pt-3 dark:border-[#1F2A44]">
                ...
                <Link href={`/subject/${doc.subject?.toLowerCase().replace(/ /g, '-') || 'unknown'}/module-${doc.module_id || 1}/${doc.id}`} onClick={() => trackDocumentStat(doc.id, 'view')} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[11px] font-bold bg-indigo-500 text-white py-2 rounded-xl">
                  <Eye size={12} /> Resume
                </Link>
              </div>
            </article>
