...
import { supabase, getRecentStudyActivity, trackDocumentStat, logRecentStudyActivity } from "../lib/api";
import { Clock, Eye, Download, FileText, Loader2, NotebookPen, FileQuestion, ListChecks } from "lucide-react";
...
export default function ContinueStudyingPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      // DEFENSIVE CHECK: Guarantee state is always an array
  ...
  // DEFENSIVE RENDER CHECK: Ensure documents is a valid array before calculating length
    ...
    <div className="space-y-6 animate-fade-up max-w-6xl mx-auto">
      <div className="rounded-3xl border border-indigo-500/20 bg-indigo-500/5 p-6 shadow-sm flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-indigo-700 dark:text-indigo-400 mt-1">Your recently accessed study materials</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {loading ? (
          ...
          return (
            <article key={doc.id} className="group flex flex-col rounded-2xl border border-[#E5E7EB] bg-white p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:border-indigo-500 dark:border-[#1F2A44] dark:bg-[#111827]">
              <div className="flex items-start justify-between">
                <div className="h-9 w-9 bg-indigo-500/10 text-indigo-500 rounded-xl flex items-center justify-center"><Icon size={16} /></div>
              ...
              <div className="mt-4 flex gap-2 border-t pt-3 dark:border-[#1F2A44]">
                ...
                <Link href={`/subject/${doc.subject?.toLowerCase().replace(/ /g, '-') || 'unknown'}/module-${doc.module_id || 1}/${doc.id}`} onClick={() => { trackDocumentStat(doc.id, 'view'); logRecentStudyActivity(doc); }} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[11px] font-bold bg-indigo-500 text-white py-2 rounded-xl">
                  <Eye size={12} /> Resume
                </Link>
              </div>
            </article>
        ...
        {safeDocuments.length === 0 && !loading && (
          <p className="col-span-full text-center py-12 text-xs text-[#64748B]">You have no recent study activity.</p>
        )}
      </div>
    </div>
  );
