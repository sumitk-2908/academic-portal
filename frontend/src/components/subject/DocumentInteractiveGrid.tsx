...
import { trackDocumentStat, deleteDocument, supabase } from "@/app/lib/api";
...
export default function DocumentInteractiveGrid({ 
  ...
  // Sync initialDocuments if they change (e.g., from tab switching)
  useEffect(() => {
  ...
  useEffect(() => {
    const initClientState = async () => {
      // Load Bookmarks from local storage safely after hydration
